...
void rip_route_description_print(const struct rip_route_description *descr, FILE *file)
{
	...
	rip2_entry_print(descr->entry, file);
	fprintf(file, "\n");
}
...
void rip2_entry_ntoh(struct rip2_entry *r2e)
{
	...
	r2e->route_tag	       = ntohs(r2e->route_tag);
...
void rip2_entry_hton(struct rip2_entry *r2e)
{
	...
	r2e->route_tag	       = htons(r2e->route_tag);
	r2e->metric	       = htonl(r2e->metric);
}

void rip_header_print(struct rip_header r_h)
{
	printf("{\n");
...
void rip2_entry_print(struct rip2_entry e, FILE *file)
{
	rip2_entry_ntoh(&e);
	char str[INET_ADDRSTRLEN];

	fprintf(file, "rfamily_id %" PRId16 ", ", e.routing_family_id);
	fprintf(file, "rtag %" PRId16 ", ", e.route_tag);

	inet_ntop(AF_INET, &(e.ip_address), str, INET_ADDRSTRLEN);
	int prefix_len = get_prefix_len(e.subnet_mask);
	fprintf(file, "network %s/%d, ", str, prefix_len);

	inet_ntop(AF_INET, &(e.next_hop), str, INET_ADDRSTRLEN);
	fprintf(file, "nh %s, ", str);
	fprintf(file, "metric %" PRId8 "", e.metric);
}
